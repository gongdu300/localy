...
async def update_profile(request: UserUpdateRequest, db: Session = Depends(get_db)):
    ...
    # [Fix] 페르소나 정보는 Persona 테이블에 저장
    persona_fields = ['persona_like_food', 'persona_hate_food', 'persona_theme',
                     'persona_like_region', 'persona_avoid_region', 'persona_transportation',
                     'persona_travel_budget', 'persona_accommodation_type',
                     'non_preferred_food', 'non_preferred_region']  # Legacy fields
    
    has_persona_update = any(getattr(request, field, None) is not None for field in persona_fields)
    
    if has_persona_update:
        # 2. 페르소나 레코드 조회 (없으면 생성)
        from models import Persona
        persona = db.query(Persona).filter(Persona.user_seq_no == user.user_seq_no).first()
        
        if not persona:
            # 신규 생성
            persona = Persona(
                user_seq_no=user.user_seq_no,
                persona_id=f"PERSONA_{user.user_id}",
                persona_like_food=getattr(request, 'persona_like_food', '') or '',
                persona_hate_food=getattr(request, 'persona_hate_food', '') or getattr(request, 'non_preferred_food', '') or '',
                persona_theme=getattr(request, 'persona_theme', '') or '',
                persona_like_region=getattr(request, 'persona_like_region', '') or '',
                persona_avoid_region=getattr(request, 'persona_avoid_region', '') or getattr(request, 'non_preferred_region', '') or '',
                persona_transportation=getattr(request, 'persona_transportation', '') or 'public_transport',
                persona_travel_budget=int(getattr(request, 'persona_travel_budget', 0)) if getattr(request, 'persona_travel_budget', None) else 300000,
                persona_accommodation_type=getattr(request, 'persona_accommodation_type', '') or 'hotel'
            )
            db.add(persona)
        else:
            # 업데이트
            if getattr(request, 'persona_like_food', None):
                persona.persona_like_food = request.persona_like_food
            if getattr(request, 'persona_hate_food', None):
                persona.persona_hate_food = request.persona_hate_food
            elif getattr(request, 'non_preferred_food', None):
                persona.persona_hate_food = request.non_preferred_food
                
            if getattr(request, 'persona_theme', None):
                persona.persona_theme = request.persona_theme
                
            if getattr(request, 'persona_like_region', None):
                persona.persona_like_region = request.persona_like_region
            if getattr(request, 'persona_avoid_region', None):
                persona.persona_avoid_region = request.persona_avoid_region
            elif getattr(request, 'non_preferred_region', None):
                persona.persona_avoid_region = request.non_preferred_region
                
            if getattr(request, 'persona_transportation', None):
                persona.persona_transportation = request.persona_transportation
            if getattr(request, 'persona_travel_budget', None):
                persona.persona_travel_budget = int(request.persona_travel_budget)
            if getattr(request, 'persona_accommodation_type', None):
                persona.persona_accommodation_type = request.persona_accommodation_type
