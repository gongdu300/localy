...
// 화면 단계를 정의합니다
type PageState = 'home' | 'auth' | 'chat' | 'reveal' | 'complete' | 'workspace' | 'tutorial' | 'dashboard';
...
export default function App() {
  ...
  // 매핑된 캐릭터 저장 (여행 일정 짜기 및 플로팅 챗봇용)
  const [matchedCharacter, setMatchedCharacter] = useState<'cat' | 'dog' | 'otter'>('otter');
  ...
  const handleRevealComplete = () => {
    // 매핑된 캐릭터 저장
    if (characterData) {
      setMatchedCharacter(characterData.character);
    }
    setPageState('dashboard');
  };

  // 5.6. 페르소나 재작성 -> 채팅 화면으로
  const handleRetryPersona = () => {
    setCharacterData(null);
    setPageState('chat');
  };

  // 6. 완료 화면에서 시작하기 -> workspace로
  ...
  // 5. 채팅 완료 시 -> 캐릭터 공개 화면으로 (characterData가 있으면) 또는 완료 화면으로
  const handleChatComplete = (charData?: typeof characterData) => {
    if (charData) {
    ...
    <div className="app-container-wrapper">
      <div className="app-container">
        <AnimatePresence mode="wait">
          ...
          {/* 축하 화면 */}
          {pageState === 'reveal' && characterData && (
            <CharacterReveal
              character={characterData.character}
              mbtiTraits={characterData.mbtiTraits}
              reason={characterData.reason}
              onComplete={handleRevealComplete}
              onRetry={handleRetryPersona}
            />
          )}
